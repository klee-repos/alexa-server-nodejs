...
(function (ts) {
    ...
    ts.version = ts.versionMajorMinor + ".0-dev.20171104";
})(ts || (ts = {}));
(function (ts) {
...
(function (ts) {
    ...
    function getJSDocTypeTag(node) {
        var tag = getFirstJSDocTag(node, 285);
...
(function (ts) {
    ...
    function isJSDocPropertyLikeTag(node) {
        return node.kind === 288 || node.kind === 283;
    }
...
(function (ts) {
    ...
    function isDeclaration(node) {
        if (node.kind === 145) {
            return node.parent.kind !== 286 || ts.isInJavaScriptFile(node);
        }
    ...
    function isJSDocTag(node) {
        return node.kind >= 280 && node.kind <= 288;
    }
...
(function (ts) {
    ...
    (function (Parser) {
        ...
        function currentNode(parsingContext) {
            ...
            var nodeContextFlags = node.flags & 6387712;
        ...
        function parseDeclaration() {
            ...
            var decorators = parseDecorators();
            var modifiers = parseModifiers();
            if (ts.some(modifiers, function (m) { return m.kind === 124; })) {
                for (var _i = 0, modifiers_1 = modifiers; _i < modifiers_1.length; _i++) {
                    var m = modifiers_1[_i];
                    m.flags |= 2097152;
                }
                return doInsideOfContext(2097152, function () { return parseDeclarationWorker(fullStart, decorators, modifiers); });
            }
            else {
                return parseDeclarationWorker(fullStart, decorators, modifiers);
            }
        }
        function parseDeclarationWorker(fullStart, decorators, modifiers) {
            switch (token()) {
                case 104:
                case 110:
                case 76:
        ...
        (function (JSDocParser) {
            function parseJSDocTypeExpressionForTests(content, start, length) {
                ...
                sourceFile = createSourceFile("file.js", 5, 1, false);
            ...
            function parseJSDocCommentWorker(start, length) {
                ...
                function parseNestedTypeLiteral(typeExpression, name) {
                    if (typeExpression && isObjectOrObjectArrayTypeReference(typeExpression.type)) {
                        ...
                        if (children) {
                            jsdocTypeLiteral = createNode(279, start_2);
                ...
                function parseClassTag(atToken, tagName) {
                    var tag = createNode(282, atToken.pos);
                    tag.atToken = atToken;
                    tag.tagName = tagName;
                ...
                function parseTypedefTag(atToken, tagName) {
                    var typeExpression = tryParseTypeExpression();
                    skipWhitespace();
                    var typedefTag = createNode(287, atToken.pos);
                    ...
                    if (!typeExpression || isObjectOrObjectArrayTypeReference(typeExpression.type)) {
                        ...
                        while (child = tryParse(function () { return parseChildParameterOrPropertyTag(0); })) {
                            if (!jsdocTypeLiteral) {
                                jsdocTypeLiteral = createNode(279, start_3);
                            }
                            if (child.kind === 285) {
                                if (childTypeTag) {
                                    break;
